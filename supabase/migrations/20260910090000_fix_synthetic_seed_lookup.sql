...
begin
  ...
  if v_clinic is null then
    -- Postgres has no min() for uuid; comparing the text form keeps the
    -- "exactly one candidate" check and picks a stable one when there is one.
    select count(*), min(id::text)::uuid into v_candidates, v_clinic
