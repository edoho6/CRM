...
export interface PatientDocument {
  ...
  /** True until the file is stored; still true afterwards means the upload failed half-way. */
...
/**
 * `sending`: a sender has claimed the row (claim_queued_messages). `stalled`: a
 * claim that never finished — it may have gone out, so a person decides.
 */
export type MessageStatus = 'queued' | 'sending' | 'sent' | 'failed' | 'skipped' | 'stalled';
/**
...
export interface MessageLogEntry {
  ...
  created_at: string;
  sent_at: string | null;
  /** When a sender claimed it, and which one. */
...
export interface DispensingRecord {
  ...
  /** Generated by the form; a retry with the same key returns the first dispensing. */
...
export interface OrderListEntryWithTarget extends OrderListEntry {
  herb: Pick<Herb, 'id' | 'pinyin_name' | 'chinese_name' | 'english_name' | 'default_unit'> | null;
  formula: Pick<HerbFormula, 'id' | 'name_pinyin' | 'name_chinese' | 'name_english'> | null;
  supplier: Pick<Supplier, 'id' | 'name'> | null;
}
...
/** Who treats whom (migration 78): filled by the diary and the record; `manual` rows by the owner. */
export interface PatientPractitioner {
  clinic_id: string;
  patient_id: string;
  user_id: string;
  source: 'appointment' | 'encounter' | 'manual';
  created_at: string;
}

/** The clinic a person chose to work in (migration 76); written only by `set_active_clinic`. */
...
/** One question about the clinic's own data and what it cost — never the question itself. */
export interface AssistantQuery {
  id: string;
  user_id: string;
  clinic_id: string;
  asked_at: string;
  status: 'answered' | 'refused_quota' | 'error';
  model: string | null;
