...
- **מנועי חיפוש:** `app/robots.ts` בשתי האפליקציות. במערכת מותר לאינדוקס רק דף הבית הציבורי
  (`/about`, קבוצת `(site)`: מה המערכת, כניסה והרשמה, בלי סשן ובלי נתונים) ודף הזימון (`/he/book/`,
  `/en/book/`), עם הקבצים שנדרשים לציור שלהם; כל השאר חסום, ודף אישור ההגעה מקבל גם `noindex` משלו
