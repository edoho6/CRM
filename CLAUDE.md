...
- **הספרייה המקצועית (RAG, migration 45):** `library_sources`/`library_chunks` (pgvector, Voyage `voyage-3-large`
  ...
  (כל `[n]` נשלף, יש ציטוט, כל מספר בקטעים) → קריאה שנייה ששופטת → **תיקון אחד** (הטיוטה חוזרת למודל עם מה
  שהבדיקות פסלו, `repairPrompt`, ונכתבת מחדש בלעדיו — ואז שוב שתי הבדיקות; פסילה שנייה סופית, ותשובה שלא נבדקה
  לעולם לא יוצאת) → `library_log_query` (מי/מתי/מקורות, **בלי טקסט**; אין policies לשינוי). לסוכן אין כלים ואין גישה לטבלאות הקליניקה; ההיסטוריה ב-`sessionStorage` בלבד.
