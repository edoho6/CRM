...
2. בטרמינל של VS Code, עם פרטי הכניסה של **החשבון שלך** (הוא אדמין הפלטפורמה; רק הוא רשאי):

   ```
   $env:MED_IMPORT_EMAIL='האימייל שלך'; $env:MED_IMPORT_PASSWORD='הסיסמה שלך'; node scripts/medicine/import.mjs
   ```

   הפקודה קוראת את `supabase/seed/medicine/dataset.json.gz` שנמצא ב-git ומדפיסה כמה ערכים
   וקישורים נטענו. הסיסמה נשארת בחלון הטרמינל הזה בלבד; לסגור אותו בסיום.
