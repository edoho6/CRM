...
5. **טעינה גדולה — האינדקס יורד ועולה.** לפני טעינה של מאות קבצים: SQL editor → `40_library_index_off_to_run.sql`
   ...
   אחרי שהטעינה הסתיימה: `41_library_index_on_to_run.sql` → Run (בונה את האינדקס פעם אחת; יכול לקחת כמה דקות).
   עד שהוא נבנה החיפוש עובד, רק לאט יותר. תוספות קטנות אחר כך (כמה קבצים) לא צריכות את זה.
