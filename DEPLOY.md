...
## סליקה דרך Grow — הכתובת שמקבלת את התשובה

המערכת לא רואה מספרי כרטיס: המטופל משלם בדף של Grow, ו-Grow מודיע למערכת מה קרה. את
ההודעה הזו מקבלת **פונקציה שרצה בתוך Supabase**, לא האתר. הסיבה: לסמן תשלום כ"שולם" זו
פעולה שדורשת הרשאה שהאתר בכוונה לא מחזיק. עד עכשיו האתר קיבל אותה עם המפתח הציבורי —
אותו מפתח שיש לכל דפדפן — וזה נסגר.

צריך לעשות את זה פעם אחת, לפני התשלום הראשון:

1. ב-SQL editor: `60_payments_and_queues_locked_down_to_run.sql`, ואחריו
   `supabase/tests/tenant_isolation.sql` (צריך להסתיים ב-ALL TENANT ISOLATION CHECKS PASSED).
2. להעלות את הפונקציה. ב-VS Code: Terminal ← New Terminal (אם כבר עשית `login` ו-`link`
   בסעיף אחר, מספיקה השורה האחרונה):

   ```
   npx supabase@latest login
   npx supabase@latest link --project-ref XXXXXXXX
   npx supabase@latest functions deploy grow-webhook --no-verify-jwt
   ```

3. במערכת: **חשבוניות ← הגדרות תשלום**. בכרטיס מופיעה עכשיו הכתובת החדשה (היא נגמרת
   ב-`/functions/v1/grow-webhook`). מעתיקים אותה ומדביקים אותה בחשבון Grow בשדה של כתובת
   ההודעות (notifyUrl / "כתובת לעדכון סטטוס").

**אין סוד בכתובת הזו, וזה בכוונה.** מה שמאשר שההודעה באמת מ-Grow הוא אסימון שהם הנפיקו
כשנוצר התשלום ושנשמר אצלנו באותו רגע — הוא לא עובר בדפדפן של המטופל ולא מתפרסם בשום
מקום. הודעה בלי האסימון המתאים לא משנה כלום. סוד בתוך כתובת היה נרשם בכל יומן שרת בדרך.
...
## מאגר המידע — קטלוג הצמחים, הפורמולות והנקודות

הקטלוג (376 צמחים, 170 פורמולות, 361 נקודות דיקור) יושב פעם אחת במערכת, וכל קליניקה מעתיקה
אותו לרשימות שלה בלחיצה. שלושה צעדים, פעם אחת:

1. ב-Supabase → SQL editor להריץ את `56_reference_catalogue_to_run.sql` (הטבלאות המשותפות,
   הכפתור, האישור של אדם ומיקומי הנקודות על הגוף התלת-ממדי), ואחריו את
   `supabase/tests/reference_catalogue.sql` ואת `supabase/tests/tenant_isolation.sql` — שניהם
   צריכים להסתיים ב-PASSED.
2. באותו עורך להריץ את `2_herbs_and_formulas_to_run.sql` ואז את `3_points_to_run.sql` (כל אחד
   לוקח כמה שניות). אם הקבצים חסרים בתיקייה, בטרמינל: `node scripts/bundle-seeds.mjs` מייצר אותם
   מחדש. הריצה ממלאת את הקטלוג המשותף בלבד; שום קליניקה לא משתנה עדיין.
3. במערכת: הגדרות ← הכרטיס "מאגר המידע" ← "טעינת הקטלוג". הכרטיס מראה כמה יש בקטלוג וכמה
   בקליניקה, ואחרי הלחיצה כמה נוסף. מה שכבר היה בקליניקה נשאר; רק שדות ריקים מתמלאים, ותיקון
   שנעשה ביד לא נדרס. אפשר ללחוץ שוב בכל עת. קליניקה חדשה מקבלת את הקטלוג לבד ביום שהיא נוצרת.

מה שנטען מסומן "דורש אימות" עד שמישהו קורא ומאשר: כפתור "אישור" ליד הסימון בדף הצמח, הפורמולה
או הנקודה רושם מי אישר ומתי; ברשימת הנקודות המסנן "דורש אימות" הוא תור הסקירה.

**מיקום נקודות על הגוף התלת-ממדי** נעשה מדף טיפול: מי שברשימת אדמיני הפלטפורמה רואה בפאנל
הגוף כפתור "מיקום נקודות" — בוחרים נקודה, לוחצים על הדגם, שומרים. המיקום משותף לכל הקליניקות,
ו"אומת" מסמנים רק אחרי בדיקה מול מקור אנטומי.
