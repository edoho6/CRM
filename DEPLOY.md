...
4. **Supabase → Edge Functions → Secrets** — ארבעה ערכים:
   ...
   (WhatsApp לא צריך secret נוסף: אותו token, והמספר של הקליניקה נכנס בהגדרות — סעיף ב.)
...
2. **המספר במערכת**: הגדרות ← הודעות ← "קו ה-WhatsApp של הקליניקה" ← המספר כפי שאומת (אפשר להקליד
   050-1234567; נשמר כ-972501234567). ממנו יוצאות ההודעות, ואליו משויכות ההודעות שמגיעות.
3. **תבניות**: באתר 019 יוצרים תבנית לכל הודעה, ובכל אחת המשתנים **בסדר הזה בדיוק** (ההודעה נשלחת עם
   ...
   | פתיחת שיחה | {{1}} שם, {{2}} קליניקה — הנוסח מוצג בכרטיס "קו ה-WhatsApp" בהגדרות |
   ...
   תזכורת, מעקב ופתיחת שיחה — Utility; יום הולדת, חזרה וחוות דעת — Marketing.
   **לתזכורת מוסיפים שני כפתורי תשובה מהירה** (Quick reply) בתבנית: "אגיע" ו-"לא אגיע". לחיצה של
   המטופל מגיעה למערכת ומעדכנת את התור ביומן לבד (סעיף ג).
4. **מזהים**: כשתבנית מאושרת, מעתיקים את המזהה שלה ומדביקים ב"הגדרות ← הודעות" בשדה "מזהה תבנית
   WhatsApp" של אותה הודעה (תבנית הפתיחה — בכרטיס "קו ה-WhatsApp"). בלי מזהה, ההודעה נשלחת כטקסט
   חופשי — ולמי שלא כתב לקליניקה לאחרונה היא תיכשל עם "צריך תבנית", ותחכה במסך "הודעות" לשליחה ביד.
...
### ג · שיחות WhatsApp בתוך המערכת (תיבת השיחות)

מה שמטופלים כותבים לקו הקליניקה מגיע למסך "הודעות" ← "שיחות", ומשם עונים; לחיצה של מטופל על "אגיע" /
"לא אגיע" בתזכורת מעדכנת את התור לבד ומחזירה לו אישור. שלושה צעדים, אחרי סעיפים א ו-ב:

1. **SQL**: SQL editor → `55_whatsapp_inbox_to_run.sql` → Run, ואז `supabase/tests/whatsapp_inbox.sql`
   (`ALL WHATSAPP INBOX CHECKS PASSED`) ושוב `supabase/tests/tenant_isolation.sql`.
2. **הפונקציה שמקבלת**: Supabase → Edge Functions → Secrets ← `WHATSAPP_INBOUND_SECRET` (מחרוזת אקראית
   ארוכה שממציאים עכשיו; לא לשלוח בצ'אט). בטרמינל של VS Code:

   ```
   npx supabase@latest functions deploy whatsapp-inbound --no-verify-jwt
   ```

3. **הכתובת אצל 019**: בממשק של 019, בהגדרות ה-WhatsApp, שדה ה-URL של "Push" (הודעות נכנסות ודוחות מסירה) ←
   להדביק:

   ```
   https://<מזהה-הפרויקט>.supabase.co/functions/v1/whatsapp-inbound?key=<הסוד-מצעד-2>
   ```

   (מזהה הפרויקט: Supabase → Project Settings → General → Reference ID.) אם 019 אומרים שהכתובת לא
   יכולה לכלול `?key=` — להגיד לי, יש דרך אחרת.

**מה בודקים:** שולחים הודעה מהטלפון האישי לקו הקליניקה; תוך שניות היא מופיעה ב"הודעות" ← "שיחות", עם
מספר אדום ליד "הודעות" בתפריט. עונים מהמערכת — ההודעה מגיעה לטלפון, ומתחתיה מופיעים סימני ✓ (נשלח, נמסר,
נקרא). כל עוד הקליניקה מסומנת כסביבת ניסיון, תשובות למטופלים לא יוצאות (נשארות "לא נשלח" עם הסיבה).
...
7. **אחרי הטעינה הראשונה**, פעם אחת ובסדר הזה: `45_library_search_to_run.sql`, `47_library_policies_to_run.sql`,
   ...
   `52_library_chats_to_run.sql`, `53_library_general_to_run.sql`, `54_messaging_automations_to_run.sql`, `55_whatsapp_inbox_to_run.sql` (הם migrations 49–57 ב-`supabase/migrations`; 52
