...
- **חשבון לבודקים** — בשתי החנויות יש שדה "Sign-in credentials" / "App access". הצוות:
  המייל והסיסמה של חשבון הבדיקות (הסנדבוקס). הפורטל: אותו מייל של מטופל בקליניקת
  הבדיקות; דף הכניסה של הפורטל יודע לקבל **קוד** במקום קישור מהמייל, ולבודקים בקליניקת
  בדיקות — גם סיסמה.
