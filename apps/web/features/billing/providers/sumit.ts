 ...
 * and response mapping means knowing SUMIT's actual endpoints, field names and
 ...
 *
 *   · the patient pays on SUMIT's domain — we ask for a payment page and
 ...
 *   · `verifyWebhook` must actually verify. If SUMIT signs its callbacks, check
 ...
 *
 * To finish this: fill in the three methods, add SUMIT's credential fields
 * below, and the rest of the system needs no changes — that is what the
 * registry is for.
 */
export const sumitProvider: PaymentProvider = {
  id: 'sumit',

  credentialFields: [
    { key: 'company_id', required: true, secret: false },
    { key: 'api_key', required: true, secret: true },
  ],

  isConfigured(credentials: ProviderCredentials) {
    return Boolean(credentials.company_id?.trim() && credentials.api_key?.trim());
  },
  ...
  async createPayment(
    ...
    if (!sumitProvider.isConfigured(context.credentials)) {
      throw new ProviderNotConfiguredError('sumit');
    }
    throw new ProviderNotImplementedError('sumit', 'createPayment');
  },

  async verifyWebhook(): Promise<WebhookVerification> {
    return {
      trusted: false,
      reason: 'sumit_webhook_verification_not_implemented',
    };
  },

  async parseWebhook(): Promise<PaymentUpdate> {
    throw new ProviderNotImplementedError('sumit', 'parseWebhook');
  },
};
