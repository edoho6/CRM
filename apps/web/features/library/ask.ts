...
export async function askLibrary(
  ...
  // The quota, before anything costs. A count that cannot be read is not a
  // zero: the question waits rather than spend without a limit.
  const { data: used, error: quotaError } = await db.rpc('library_questions_today');
  if (quotaError) throw new LibraryUnavailableError('quota_unreadable');
  if (Number(used ?? 0) >= LIBRARY_LIMITS.dailyQuota) {
    await log('refused_quota', []);
    return reply('refused_quota', LIBRARY_REFUSED_QUOTA_HE);
  }

  // Everything from here may spend. Whatever way it ends — an answer, a
  // refusal, or an error thrown half-way — it is logged once, with what it
  // cost: the daily ceiling counts logged rows, and a question that threw
  // after two paid calls used to count as nothing.
  let logged = false;
  const logOnce = async (status: LibraryStatus, sources: LogSource[], spent?: boolean) => {
    logged = true;
    await log(status, sources, spent);
  };
  const spentAnything = () => usage.input + usage.output + usage.cacheWrite + usage.cacheRead > 0;
  ...
  const run = async (): Promise<AskResult> => {
    ...
    if (await canonReady(db)) {
      ...
      if (!canon.answer.trim()) {
        await logOnce('no_sources', [], true);
        return reply('no_sources', LIBRARY_NO_SOURCES_HE);
      }
      await logOnce('answered', [], true);
    ...
    const generalReply = async (
      ...
      if (!general) {
        await logOnce('no_sources', logSources(new Set()), usage.input > 0);
        return reply('no_sources', LIBRARY_NO_SOURCES_HE, citations, retrieved);
      }
      await logOnce('general', logSources(new Set()), true);
      return {
    ...
    await logOnce('answered', logSources(cited), true);
    return {
  ...
  try {
    ...
    if (!logged) await log('error', [], spentAnything());
    throw error;
  }
}
