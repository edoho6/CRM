...
- **פורמולה שהומצאה בטיפול נשארת בטיפול:** שם שהוקלד ולא נבחר מהרשימה נשלח כ-`custom_formula`,
  ו-`record_prescription` שומר אותו כשורה על רשומת הניפוק (`dispensing_items.custom_name`) — לעולם
  לא כשורה חדשה ב-`herb_formulas`. הקטלוג הוא מה שהמטפל בחר לשמור, לא כל אלתור של השנה. אין טיפוס
  שאוכף את זה, ולכן `catalogue-writes.test.ts` קורא את הקוד עצמו ומוודא שהמסך היחיד שמוסיף פורמולה
  לקטלוג הוא עורך הפורמולות (`features/inventory/actions.ts`)
